...
//  load = current units assigned (not yet tracked live; defaults to 0
//         until schedule publishing is wired to compute this per faculty)
...
async function loadFacultyList() {
  try {
    ...
    facultyData = data.map(f => ({
      ...
      load:         0,   // not yet tracked live — see note above
      max:          f.max_units || 21,
      availability: null,
      preference:   null,
      status:       null,
